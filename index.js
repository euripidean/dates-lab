...
const bday = new Date("Nov, 3 1984");
...
console.log('Components of "newYear"');
...
consecutiveDatesWithUnit(new Date(2019, 0, 1), 3, 1, "year");
...
function orderDatesIntoObject(dates) {
  const orderedDates = { past: [], present: [], future: [] };
...
orderDatesIntoObject([today, dueDate, startDate, bday, newYear]);
...
function nextDate(dates) {
  ...
  console.log(`Your next appointment is ${days} days from now.`);
}
...
function whensYourParty(date, year) {
  ...
  const partyDate = new Date(year, date.getMonth(), dayOfYear);

  console.log(days[partyDate.getDay()]);
}
